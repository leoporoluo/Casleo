...
function bundledNodeAndPnpm() {
  const roots = [
    join(homedir(), 'AppData', 'Local', 'Programs', 'Casleo', 'resources', 'app', 'node_modules'),
  ...
  throw new Error('No bundled pnpm found; run from the repo or install Casleo.')
}
...
async function main() {
  ...
  if (promoted && existsSync(packageRoot) && existsSync(closure)) {
    ...
    const appClosure = join(
      homedir(),
      'AppData', 'Local', 'Programs', 'Casleo', 'resources', 'app', 'node_modules'
    )
