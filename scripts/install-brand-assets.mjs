...
const destination = path.join(destinationDirectory, 'casleo-logo.png')
...
function replaceIconLink(contents, file) {
  const desktop = '<link rel="icon" type="image/png" href="/casleo-logo.png" />'
  if (contents.includes(desktop)) return contents
  const matches = contents.match(/<link rel="icon"[^>]*>/gu) ?? []
  if (matches.length !== 1) {
    throw new Error(
      `Could not update Casleo branding in ${file}: expected one icon link, found ${String(matches.length)}`
    )
  }
...
function replaceManifestIcon(contents, file) {
  ...
  const target = icons.find((icon) => icon?.src === '/casleo-logo.png')
    ?? icons.find((icon) => typeof icon?.src === 'string' && icon.src.endsWith('favicon.svg'))
  if (target === undefined) {
    throw new Error(`Could not update Casleo branding in ${file}: no icon entry to replace`)
  }
  target.src = '/casleo-logo.png'
  target.sizes = '256x256'
...
console.log(
  `Installed Casleo brand assets: ${path.relative(projectRoot, destination)}`
)
