...
const CHECKPOINT_UNDO_ENTRY = "tether-checkpoint-undone";
const DIFF_ENTRY = "tether-diff";
...
export function createTetherExtension(options) {
    return {
        name: "tether",
        factory(pi) {
            registerTetherProjectTrust(pi);
            ...
            const applyPermissionTools = () => {
                if (permission === "plan") {
                    if (toolsBeforePlan === undefined) {
                        // Seed from Tether's allowlist, not Pi's default builtins (bash/edit/…).
            ...
            pi.on("session_start", async (_event, ctx) => {
                ...
                // Activate Tether tools first so the opening prompt is not blocked on MCP.
            ...
            pi.on("before_agent_start", async (event, ctx) => {
                ...
                return {
                    systemPrompt,
                    message: {
                        customType: "tether-plan-context",
                        display: false,
                        content: [
                            "[PLAN MODE ACTIVE]",
            ...
            const executeApprovedPlan = (ctx) => {
                ...
                applyPermissionTools();
                updateStatus(ctx);
                pi.appendEntry("tether-permission", { permission });
                pi.sendUserMessage([
            ...
            pi.registerCommand("plan", {
                ...
                handler: async (args, ctx) => {
                    ...
                    if (action === "clear") {
                        ...
                        ctx.ui.setWidget("tether-plan", undefined);
...
//# sourceMappingURL=tether-extension.js.map
