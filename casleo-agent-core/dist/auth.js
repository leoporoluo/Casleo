...
export async function hasStoredProviderCredential(providerId, authPath) {
    return isStoredCredential(await (await credentialStore(authPath)).read(providerId));
}
export function hasDeepSeekEnvironmentKey() {
    return Boolean(process.env.DEEPSEEK_API_KEY?.trim());
}
export async function saveDeepSeekKey(key, authPath) {
    await saveProviderApiKey(PROVIDER_ID, key, authPath);
}
export async function saveProviderApiKey(providerId, key, authPath) {
    const trimmed = key.trim();
    if (!trimmed)
        throw new Error(`${providerDisplayName(providerId)} API key cannot be empty`);
    await (await credentialStore(authPath)).modify(providerId, async () => ({ type: "api_key", key: trimmed }));
}
export async function removeStoredDeepSeekKey(authPath) {
    return removeStoredProviderCredential(PROVIDER_ID, authPath);
}
export async function removeStoredProviderCredential(providerId, authPath) {
    const store = await credentialStore(authPath);
    if (!(await store.read(providerId)))
        return false;
    await store.delete(providerId);
    return true;
}
export async function validateDeepSeekKey(key, baseUrl, modelId, fetchImpl = fetch) {
    try {
        ...
        const modelAvailable = isRecord(body) &&
...
export function isInteractiveInvocation(piArgs) {
    if (!process.stdin.isTTY || !process.stdout.isTTY)
        return false;
    if (piArgs.includes("--print") || piArgs.includes("-p"))
        return false;
...
export async function ensureFirstRunAuth(options) {
    ...
    const configured = Boolean(environmentKey && process.env[environmentKey]?.trim()) ||
...
export async function runAuthCommand(command, options) {
    if (command === "logout") {
        ...
        if (environmentKey && process.env[environmentKey]?.trim()) {
            process.stdout.write(`${pc.yellow("!")} ${environmentKey} is still set in this environment.\n`);
        }
        return;
    }
    if (command === "status") {
...
async function promptAndStoreKey(baseUrl, modelId) {
    while (true) {
        ...
        if (validation.status === "unverified") {
            ...
            if (!validation.modelAvailable) {
                process.stdout.write(`${pc.yellow("!")} The key works, but ${modelId} was not returned by /models.\n`);
            }
        }
...
async function loginWithProvider(providerId) {
    ...
    process.stdout.write(`${pc.green("✓")} Signed in securely. ${result.providerId}/${result.modelId} is now the default.\n`);
}
...
async function selectProviderAuthType(provider, interaction) {
    const methods = [];
...
function terminalAuthInteraction() {
    return {
        prompt: promptForAuth,
        notify: notifyAuth,
    };
}
async function promptForAuth(prompt) {
    ...
    return readLine(`${prompt.message}${prompt.placeholder ? ` [${prompt.placeholder}]` : ""}: `, prompt.signal);
}
function notifyAuth(event) {
    ...
    if (event.type === "device_code") {
        process.stdout.write(`Open ${event.verificationUri} and enter code ${pc.bold(event.userCode)}.\n`);
...
async function readLine(message, signal) {
...
export function formatBaseUrlPrompt(defaultBaseUrl) {
    return `API base URL (optional, press Enter to use default) [${defaultBaseUrl}]: `;
}
async function readSecret(prompt) {
    if (!process.stdin.isTTY)
        throw new Error("A TTY is required for secret input");
