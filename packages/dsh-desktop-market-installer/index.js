...
export async function ensurePnpmShim(home = dshHome()) {
  ...
  // Casleo's installer and the community market alike — so the runner it
  ...
  return Object.freeze({
    ...
    select: async (name) => {
      if (name !== MARKET_PROFILE) {
        throw new Error(`Casleo only exposes the ${MARKET_PROFILE} profile.`)
      }
    }
  })
}
...
export function createDesktopPnpmService(options) {
  ...
  const updateSharedMarket = async (args, spec, invokingDir, write, isCancelled, setCancel) => {
    ...
    try {
      ...
      // there, and the flag below is passed unconditionally.
      ...
      // The tail of both streams travels with the failure: pnpm's exit code
      // alone names no cause, and the caller only gets what is thrown.
      let failureOutput = ''
      ...
      setCancel(() => {})
      // A cancellation is not a failure: the market's own cancel path reads the
      // message, and the child was killed, so its exit code proves nothing.
      if (isCancelled()) throw new Error('The package operation was aborted.')
      if (exit.code !== 0) {
  ...
  const runExternalMarketPluginInstall = (args, invokingDir, signal) => {
    ...
    if (closed) throw new Error('The Casleo pnpm service has been disposed.')
