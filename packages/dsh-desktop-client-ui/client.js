window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // The plugin owns two sidebar seats and nothing else: the desktop's own
    // chrome is handled by the shell (see src/preload/windows-titlebar.ts for
    // the Windows caption strip), so no settings row and no locale dictionary
    // lives here any more.
    const inject = ['slots']
    function apply(ctx) {
