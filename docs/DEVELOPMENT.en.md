...
> A repo-oriented AI coding desktop workbench. This guide is for engineers working inside the Casleo repository: architecture, module responsibilities, key data flows, and common development tasks. Product description and user documentation live in `README.md` / `README.zh-CN.md`.
...
├── .agents/skills/            # built-in project skills (init-long-run / continue-long-run / plan-then-act / casleo-ui)
...
All styling lives in `styles.css` (hand-written CSS, no UI framework). Before changing UI look-and-feel, read `.agents/skills/casleo-ui/SKILL.md` and `tokens.md` (UI design-token conventions).
...
- Locale resolution: `resolveLocale` prefers the stored value (`locale` in `~/.casleo/settings.json`), otherwise follows the system language list; default is `zh`.
...
Skills are loaded by the Pi runtime (Casleo does not ship a separate loader); this repository only provides UI and filesystem helpers:

- **Scanning**: `src/main/skills-fs.ts` scans user-level (`~/.casleo/skills`, `~/.agents/skills`) and project-level (`.agents/skills`, `.pi/skills`, requires trusting the project) roots; `app:list-skills` feeds the settings page.
...
- **Built-in project skills**: `.agents/skills/` contains `init-long-run` / `continue-long-run` / `plan-then-act` / `casleo-ui` (UI look-and-feel). Skill files must include frontmatter `name` + `description`; missing either means the skill is not loaded.
...
Runs on postinstall / predev: checks whether the Electron distribution binaries are complete, runs `install.js` when missing, unpacks from the `~/Library/Caches/electron` zip cache on macOS, and patches `CFBundleName` / `CFBundleDisplayName` in `Info.plist` to Casleo.
...
- First read `.agents/skills/casleo-ui/SKILL.md` and `tokens.md` to keep color, radius, and spacing tokens consistent.
