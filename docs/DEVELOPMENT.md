...
- 把 `casleo-agent-core` 的 RPC 子进程桥接成桌面可用的 agent
...
Electron Main（src/main）
  窗口、工作区文件、凭据、AgentHost 子进程宿主、更新检查、预览协议
        ...
        ▼
casleo-agent-core（node_modules 依赖，RPC worker 进程）
  权限、沙箱、工具、checkpoint、MCP、会话文件、Pi agent 循环
```

渲染进程没有任何 Node.js 直连能力；桌面能力全部通过 `src/shared/types.ts` 里定义的类型化 `DesktopApi` 契约走 IPC。agent 运行在独立子进程（通过 `process.execPath` + `ELECTRON_RUN_AS_NODE=1` 启动），崩溃后磁盘上的会话仍可继续作为对话恢复，但 Casleo 不会静默重放未完成的命令。
...
├── .agents/skills/            # 项目内置技能（init-long-run / continue-long-run / plan-then-act / casleo-ui）
...
应用从 npm 消费 `casleo-agent-core`。如果要同时改 Runtime 本身，临时把本地 Runtime 包链接进来（README 建议 `../Casleo/casleo-agent-core`）。正式发布前记得改回 registry 版本。
...
agent 运行在独立的 Node 子进程里（`casleo-agent-core` 的 RPC 入口），通信协议是**换行分隔的 JSON-RPC over stdio**：
...
- 语言解析：`resolveLocale` 优先存储值（`~/.casleo/settings.json` 的 `locale`），否则跟随系统语言列表；默认 `zh`。

## 7. 会话、恢复与编辑安全

### 7.1 会话文件

会话由 `casleo-agent-core` 管理（`CasleoStateStore` / `listCasleoThreads`），主进程只做映射：`sessions:list` 把 thread 转成 `SessionSummary`（含 `path` 会话文件路径、`storagePath` 索引路径、`preview` 等）。重命名通过**追加**一条 `session_info` 条目并重建索引完成（`sessions:rename`）。
...
后台/进行中的 shell 命令由 `sessionTerminals` 从工具活动里提炼（`process_id` 追踪），InspectPanel 可 `stopJobs("/stop-job <id>")` 或 `/stop-jobs`（需要 casleo-agent-core ≥ 0.1.6 提供该命令）。
...
技能由 Pi 运行时加载（Casleo 不另写 loader），本仓库只做 UI 与文件系统辅助：

- **扫描**：`src/main/skills-fs.ts` 扫用户级（`~/.casleo/skills`、`~/.agents/skills`）与项目级（`.agents/skills`、`.pi/skills`，需信任项目）；`app:list-skills` 供设置页展示。
...
- **项目内技能**：`.agents/skills/` 下有 `init-long-run` / `continue-long-run` / `plan-then-act` / `casleo-ui`（界面气质）。技能文件必须含 frontmatter `name` + `description`，缺项不加载。
...
- `extraResources` 放 `icon.png`；macOS、Windows 与 Linux 均使用 `build/icon.png` / `build/icon-win.png`，构建配置统一维护图标。
...
postinstall / predev 执行：检查 Electron 发行二进制是否完整，缺失时跑 `install.js`，macOS 下还会从 `~/Library/Caches/electron` 缓存 zip 解压，并把 `Info.plist` 的 `CFBundleName` / `CFBundleDisplayName` 改为 Casleo。
...
- 先读 `.agents/skills/casleo-ui/SKILL.md` 与 `tokens.md`，保持配色、圆角、间距令牌一致。
