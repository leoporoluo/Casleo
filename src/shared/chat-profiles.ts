...
export function parseChatProfiles(raw: unknown): ChatProfiles | undefined {
  ...
  const listed = listedProfiles.length > 0 || Array.isArray(value.customProfiles);
  let customProfiles: CustomApiProfile[] = listed
    ? listedProfiles.map((item: unknown) => parseCustomProfile(item)).filter((item): item is CustomApiProfile => Boolean(item))
    : [];
...
export function buildCustomProfilesPayload(
  ...
  activeCustomId: string,
  draft: Pick<CustomApiProfile, "name" | "url" | "model" | "apiKey"> & Partial<Pick<CustomApiProfile, "contextWindow" | "maxTokens" | "effort">>,
): { customProfiles: CustomApiProfile[]; activeCustomId: string } {
