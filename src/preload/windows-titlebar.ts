...
/**
 * Windows draws the window controls over the page (WCO), so the page has to
 * behave like a title bar: a strip the user can drag the window by, and a
 * column on the right where the system's own buttons sit and no page control
 * may hide.
 *
 * The Harness shell already owns both places: the sidebar's logo row and the
 * session header. Rather than adding a band above them, this module makes
 * those two surfaces draggable, keeps every interactive control out of the
 * caption column, and mirrors the resolved page theme back to the main process
 * so the buttons' glyph color follows the app instead of the OS.
 */
...
/** Distance between the caption column and whatever sits next to it. */
const CAPTION_GUTTER_PX = 8
...
function installLayout(document: Document): void {
  ...
  style.textContent = `
    ...
    :root {
      --dsh-titlebar-safe-inset-top: ${WINDOWS_TITLEBAR_HEIGHT}px;
    }
    ...
    body.dsh-desktop-windows-titlebar-layout [data-side="rightbar"] {
      top: var(--dsh-titlebar-safe-inset-top, ${WINDOWS_TITLEBAR_HEIGHT}px) !important;
      height: calc(100% - var(--dsh-titlebar-safe-inset-top, ${WINDOWS_TITLEBAR_HEIGHT}px)) !important;
    }
    ...
    /*
     * The header is 76px tall and only its upper half holds the crumb row; the
     * caption buttons live in the upper-right corner of that band. The crumb
     * row therefore stops short of the caption column, and the header's own
     * right-hand clusters (utilities, corner) move down into the lower half,
     * where they share the row with the view tabs — the same slot the official
     * layout uses for tabs.
     */
    body.dsh-desktop-windows-titlebar-layout [data-slot="conversation.session.header"] > header > div:first-child {
      padding-right: calc(var(${CAPTION_WIDTH_PROPERTY}, 140px) + ${CAPTION_GUTTER_PX}px) !important;
      box-sizing: border-box !important;
    }
    ...
    body.dsh-desktop-windows-titlebar-layout [data-slot="conversation.session.header"] > header [class*="headerCorner"] {
      ...
      top: ${WINDOWS_TITLEBAR_HEIGHT + 2}px !important;
    ...
    #${DRAG_REGION_ID} {
      ...
      top: 0;
      left: 0;
      right: calc(var(${CAPTION_WIDTH_PROPERTY}, 140px) + ${CAPTION_GUTTER_PX}px);
      /*
     ...
     * the caption controls inside a draggable rectangle and make them
     * unclickable. This guard paints after every drag region and subtracts that
     * column from all of them.
     */
    #${CAPTION_GUARD_ID} {
      position: fixed;
      top: 0;
      right: 0;
      width: calc(var(${CAPTION_WIDTH_PROPERTY}, 140px) + ${CAPTION_GUTTER_PX}px);
...
function installDragRegion(document: Document): void {
  ...
  // region above it, so the system's caption controls keep the click.
