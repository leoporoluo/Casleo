...
export class AgentHost {
  ...
  async request<T>(type: string, data: Record<string, unknown> = {}): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error(`Casleo agent did not respond to ${type}. ${this.stderr}`.trim()));
  ...
  private handleLine(line: string): void {
    ...
    if (data.type === "response" && typeof data.id === "string") {
      ...
      if (data.success === false) pending.reject(new Error(String(data.error ?? "Casleo agent command failed")));
