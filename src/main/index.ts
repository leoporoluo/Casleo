...
function configureAppIdentity(): void {
  if (developmentBuild) {
    app.setName('Casleo Dev')
    app.setPath('userData', join(app.getPath('appData'), 'casleo-dev'))
    return
  }
  ...
  app.setPath('userData', join(app.getPath('appData'), 'casleo'))
}
...
function ensureTray(): void {
  ...
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label: locale === 'zh' ? '显示 Casleo' : 'Show Casleo', click: restoreMainWindow },
      { type: 'separator' },
...
async function showAbout(window: BrowserWindow): Promise<void> {
  ...
  await dialog.showMessageBox(window, {
    type: 'info',
    title: 'Casleo',
    message: locale === 'zh' ? '关于 Casleo' : 'About Casleo',
    detail: aboutDetail(
      app.getVersion(),
...
function showUnexpectedError(error: unknown): void {
  ...
  dialog.showErrorBox('Casleo encountered an error', message)
}
      ...
      ? [
        {
          ...
          submenu: [
            {
              label: isChinese ? '关于 Casleo' : 'About Casleo',
              click: () => {
