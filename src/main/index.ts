...
const userDataPath = path.join(app.getPath("appData"), "Casleo");
...
async function loadAppPreferences(): Promise<void> {
  try {
    const raw = JSON.parse(await fsp.readFile(appPreferencesPath(), "utf8")) as Partial<AppPreferences>;
    appPreferences = {
  ...
  app.setLoginItemSettings({ openAtLogin: appPreferences.openAtLogin });
}

async function saveAppPreferences(next: AppPreferences): Promise<void> {
  appPreferences = {
    minimizeToTray: next.minimizeToTray === true,
    openAtLogin: next.openAtLogin === true,
  };
  await fsp.mkdir(userDataPath, { recursive: true, mode: 0o700 });
  await fsp.writeFile(appPreferencesPath(), `${JSON.stringify(appPreferences, null, 2)}\n`, { mode: 0o600 });
  app.setLoginItemSettings({ openAtLogin: appPreferences.openAtLogin });
}
...
function createTray(): void {
  ...
  tray = new Tray(icon);
  tray.setToolTip("Casleo");
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: "显示 Casleo", click: () => { mainWindow?.show(); mainWindow?.focus(); } },
    { type: "separator" },
    { label: "退出 Casleo", click: () => { app.quit(); } },
  ]));
...
function registerIpc(): void {
  ...
  ipcMain.handle("app:preferences", () => appPreferences);
  ipcMain.handle("app:save-preferences", async (_event, next: Partial<AppPreferences>) => {
  ...
  ipcMain.handle("workspace:rename", async (_event, workspacePath: string, name: string) =>
    recentWorkspaces.rename(workspacePath, name),
  );
  ipcMain.handle(
    "workspace:read",
...
const recentWorkspaces = {
  ...
  async touch(workspacePath: string): Promise<string> {
    ...
    const existing = current.find((item) => item.path === resolved);
    const next = [
      {
        path: resolved,
        name: existing?.name || path.basename(resolved) || resolved,
  ...
  async rename(workspacePath: string, name: string): Promise<WorkspaceItem[]> {
    ...
    const current = await this.list();
    if (!current.some((item) => item.path === resolved)) throw new Error("Project is not in recent workspaces");
    const next = current.map((item) => item.path === resolved ? { ...item, name: nextName } : item);
    await fsp.writeFile(recentFile, `${JSON.stringify(next, null, 2)}\n`, { mode: 0o600 });
    return next;
  },
};
