...
describe('agent preset package transfer', () => {
  ...
  it('adds import preview, conflict rename, trust warning, and custom-card export controls', async () => {
    ...
    // The desktop preset surface is local-only: no link leaves for a
    // third-party preset gallery.
    expect(patch).not.toContain('awesomePreset')
    expect(patch).not.toContain('dshdesktop')
    expect(patch).not.toContain('"_blank", "noopener,noreferrer"')
